...
#include "display.h"
#include "font8x8.h"
#include "font_hd44780.h"

#define FRAMEBUFFER_SIZE 32 * 128 /8
...
//    SSD1306_drawText(font_8x8,text, anchor_x,anchor_y,SSD1306_ADD,SSD1306_DEG0);             
    SSD1306_drawText(hd44780font8x8,text, anchor_x,anchor_y,SSD1306_ADD,SSD1306_DEG0);             
    SSD1306_sendBuffer();
}
...
void SSD1306_drawChar8x8(char c, uint8_t anchor_x, uint8_t anchor_y){
      SSD1306_drawChar(hd44780font8x8,c, anchor_x , anchor_y, SSD1306_ADD,SSD1306_DEG0 );
}            

void SSD1306_drawChar8x10(char c, uint8_t anchor_x, uint8_t anchor_y){
      SSD1306_drawChar(hd44780font8x10,c, anchor_x , anchor_y, SSD1306_ADD,SSD1306_DEG0 );
}            
...
//        uint16_t seek = (c - 32) * (font_width * font_height) / 8 + 2;
        uint16_t seek = c* (font_width * font_height) / 8 + 2;
        ...
        for (uint8_t y = 0; y < font_height; y++) {
           for (uint8_t x = 0; x < font_width; x++) {
                if (font[seek] >> b_seek & 0b00000001) {
                    switch (rotation) {
                        case SSD1306_DEG0:
                            SSD1306_setPixel(7-x + anchor_x, y + anchor_y, mode);
                            break;
                        case SSD1306_DEG90:
...
//origional for 8x8 font
void SSD1306_drawCharX(const unsigned char *font, char c, uint8_t anchor_x, uint8_t anchor_y,
                  enum SSD1306_WriteMode mode, enum SSD1306_Rotation rotation) {
        if( !font || c < 32) return;
